...
namespace vks 
{
	...
	{
		bool res = ImGui::InputFloat(caption, value, step, step * 10.0f, "%g");
		if (res) { updated = true; };
		return res;
	}
    ...
    bool UIOverlay::inputText(const char* caption, char * text, size_t size) {
        bool res = ImGui::InputText(caption, text, size);
        if (res) { updated = true; };
        return res;
